...
#define NEART_INSTR_FORECH(F) \
    F(0x0, N_END,   _,                  instr_end,\
      "end the execution") \
    \
    F(0x1, NI_LSI,  NI_LOAD_STACK_INT,  instr_int_load_stack,\
      "load a 32 bit integer off the stack into the register x") \
    \
    ...
    F(0x3, NR_SUB,  _,                  instr_reg_sub, \
      "add to registers into a target register (32 bit)") \
    \
    F(0x4, NI_SPI,  NI_STACK_PUSH_INT,  instr_int_stack_push,\
...
#define INSTR_DECLARE_SHORT(p1, p2, ...) const rcode_t p2;
#define INSTR_DEFINES_SHORT(p1, p2, ...) const rcode_t p2 = ((rcode_t)p1);
